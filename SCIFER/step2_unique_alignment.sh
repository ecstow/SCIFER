...
#Usage: for i in *.fastq; do bash Step2_unique_alignment.sh $i; done

#This script aligns R2 uniquely to the hg38 genome, deduplicates reads based on UMI, and determines the number of reads indicating L1 expression.
...
FL_L1s_minus='SCIFER/annotations/FL-L1-BLAST_RM_minus_hg38_1_2021.bed'

prefix=${1%.fastq}  

#Perform unique alignment using bowtie-0.12.8
$bowtie -p 10 -m 1 -S -y -v 3 --chunkmbs 8184 $hg38_index ${prefix}.fastq | samtools view -hbuS - | samtools sort -o ${prefix}_bowtie_hg38_sorted.bam
...
while read line in file; do grep $line ${prefix}_bowtie_hg38_sorted_rmdup_top_bottom.sam > ${line}_barcode.sam; done < $barcodes
...
for file in sam_files/*.sam; do (head -30 $header ; cat $file) > ${file}_header.sam; done
...
| head -1  > sam_files_2/${line}_barcode.sam; done < $barcodes
for file in sam_files_2/*.sam; do (head -30 $header ; cat $file) > ${file}_header.sam; done
...
mkdir read_counts
$bedtools coverage -abam $FL_L1s_plus -b sam_files_2/${file}_top.bam > read_counts/${file}_plus.txt
$bedtools coverage -abam $FL_L1s_minus -b sam_files_2/${file}_bottom.bam > read_counts/${file}_minus.txt
